...
def is_legal(m: Move, b: Board) -> bool:
    ...
    board. The move must be between 1 and 25.
    """
    legal = False
    if (b.board[target(m)] == empty_space
        and ((white_plays(b) and b.board[source(m)] == white_piece)
             or (not white_plays(b) and b.board[source(m)] == black_piece))):
        if white_plays(b):
            return _legal_white(m, b.board)
        else:
            _legal_black(m, b.board)
        if (_row(source(m)) - _row(target(m)) in (-2, 0, 2)
            ...
            _legal_attack(m, b.board)
    return legal

def _legal_white(m: Move, b: Board) -> None:
    """Check if it is a legal move for the white player."""
    legal = False
...
def _legal_black(m: Move, b: Board) -> None:
    """Check if it is a legal move for the black player."""
    legal = False
...
def _legal_attack(m: Move, b: Board) -> None:
    """Check if it is a legal attack for a given player."""
    legal = False
    if (source(m) % 2 == 1
